...
for f in glob.glob('*.html'):
    ...
    if f == 'index.html':
        ...
        # Standard Tailwind header replacement (everything inside the last <div class="flex items-center gap-3"> in <header>)
        # Actually, let's just find the first occurrence of '<div class="flex items-center gap-3">' and replace it up to '</header>' 
        # Wait, the sidebar has one too, or the top left.
        # Let's search inside the <header> block.
        def replace_header(m):
            header_content = m.group(1)
            # Find the last <div class="flex items-center gap-3"> in the header
            last_div = header_content.rfind('<div class="flex items-center gap-3">')
            if last_div != -1:
                return '<header' + m.group(0).split('<header', 1)[1][:m.start(1)-m.start(0)] + header_content[:last_div] + html_header_right + '\n        </header>'
            return m.group(0)
            
        content = re.sub(r'<header[^>]*>([\s\S]*?)</header>', replace_header, content)
